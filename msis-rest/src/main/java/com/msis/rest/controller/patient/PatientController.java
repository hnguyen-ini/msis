...
public class PatientController implements InitializingBean {
	...
	public Response create(Patient patient, @QueryParam("accessToken") String accessToken) {
		ServiceResponse<Patient> response = new ServiceResponse<Patient>();
		try {
			log.info("Create Patient " + JsonHelper.toString(patient));
			patient = patientService.create(patient, accessToken);
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response update(Patient patient, @QueryParam("accessToken") String accessToken) {
		ServiceResponse<Patient> response = new ServiceResponse<Patient>();
		try {
			log.info("Update Patient " + JsonHelper.toString(patient));
			patient = patientService.update(patient, accessToken);
		} catch (ServiceException e) {
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response getByCreator(@PathParam("creator") String creator, @QueryParam("accessToken") String accessToken) {
		ServiceResponse<List<Patient>> response = new ServiceResponse<List<Patient>>();
		try {
			log.info("Get Patient by creator " + creator);
			List<Patient> patients = patientService.findByCreator(creator, accessToken);
			if (patients.size() == 0) {
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Produces(MediaType.APPLICATION_JSON)
	public Response search(@PathParam("search") String search, @QueryParam("accessToken") String accessToken) {
		ServiceResponse<List<Patient>> response = new ServiceResponse<List<Patient>>();
		try {
			List<Patient> patients = patientService.findByNameOrIDn(search, accessToken);
			response.setResult(patients);
		} catch(ServiceException e) {
